...
function renderFooter() {
    ?>
        ...
        <script>
            ...
            // Open new reservation modal — redirects to walk-in when offline
            function openNewReservation() {
                if (!navigator.onLine || window._staffOfflineMode) {
                    <?php if ($currentRestaurant !== 'all'): ?>
                    openWalkinModal();
                    <?php else: ?>
                    showToast('Sin conexión — Selecciona un restaurante específico para registrar walk-ins.', 'warning');
                    <?php endif; ?>
                    return;
                }
            ...
            // ── Walk-in modal (offline-safe quick reservation) ─────────────────
            ...
            async function submitWalkin() {
                ...
                var alertEl = document.getElementById('walkinAlert');

                if (!party || party < 1) {
                    alertEl.textContent = 'Ingresa el número de personas.';
                    alertEl.className = 'mb-4 px-4 py-3 rounded-lg text-sm font-medium bg-red-900/40 border border-red-700/40 text-red-300';
                    return;
                }
                if (!time) {
                    alertEl.textContent = 'Ingresa la hora del walk-in.';
                    alertEl.className = 'mb-4 px-4 py-3 rounded-lg text-sm font-medium bg-red-900/40 border border-red-700/40 text-red-300';
                    return;
                }
                ...
                alertEl.className = 'hidden mb-4 px-4 py-3 rounded-lg text-sm font-medium';
